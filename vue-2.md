...
### 2.1 常用用法
场景:表格初始进来需要调查询接口 getList(),然后input 改变会重新查询
...
### 3.1 props
这个应该非常熟悉,就是父传子的属性; props 值可以是一个数组或对象;
...
props:{
 inpVal:{
  ...
  required: true, //是否必传
  default:'success',  //默认值,对象或数组默认值必须从一个工厂函数获取如 default:()=>[]
  validator:(value) {
    // 这个值必须匹配下列字符串中的一个
...
### 3.2 $emit
这个也应该非常常见,触发子组件自定义事件触发父组件给自己绑定的事件,其实就是子传父的方法
...
1.$attrs场景:如果父传子有很多值,那么在子组件需要定义多个 props
解决:$attrs获取父传子中未在 props 定义的值
...
````
但是这样每次组件都会重新加载,会消耗大量性能,所以keep-alive就起到了作用
...
````js
// 递归组件: 组件在它的模板内可以递归的调用自己，只要给组件设置name属性就可以了。
// 那么在组件模板内就可以递归使用了,不过需要注意的是，
